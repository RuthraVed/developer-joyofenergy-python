...
class PricePlanService:
    def __init__(self, reading_repository):
        self.electricity_reading_service = ElectricityReadingService(
            reading_repository)

    def get_list_of_spend_against_each_price_plan_for(self, smart_meter_id, limit=None):
        readings = self.electricity_reading_service.retrieve_readings_for(
            smart_meter_id)
        if len(readings) < 1:
            return []
        ...
        list_of_spend = list(
            map(cost_from_plan, self.cheapest_plans_first(price_plans)))
    ...
    def calculate_average_reading(self, readings):
        sum = reduce((lambda p, c: p + c),
                     map(lambda r: r.reading, readings), 0)
        return sum / len(readings)

    # To get plan details from price_plan_repository
    def get_plan_details(self, plan_id):
        price_plans = price_plan_repository.get()
        plan = list(filter(lambda plan: plan.name == plan_id, price_plans))
        return plan[0]
